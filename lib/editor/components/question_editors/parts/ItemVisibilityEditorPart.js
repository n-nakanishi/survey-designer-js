...
class ItemVisibilityEditorPart extends Component {
  ...
  choiceReferenceRenderer(instance, td, row, col, prop, value) {
    ...
    const choiceId = Replacer.extractIdFrom(value);
    const od = this.findOutputDefinitionFromRow(row);
    const choice = od.getChoices().find(choice => choice.getId() === choiceId);
    if (choice) td.textContent = choice.getLabel();
    else td.textContent = '不正な値です'
    return td;
  }
  ...
  radioSelectOptions(outputDefinition) {
    ...
    choices.forEach((choice) => {
      options[question.getChoiceReference(choice)] = choice.getLabel();
    });
    return options;
  }
  ...
  comparisonTypeSelectOptions(row) {
    ...
    switch (outputDefinition.getOutputType()) {
      case 'number':
        return COMPARISON_TYPE_OPTIONS;
      default:
        return emptyOptions;
    }
  }
