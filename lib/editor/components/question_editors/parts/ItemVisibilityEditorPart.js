...
class ItemVisibilityEditorPart extends Component {
  ...
  handleChangeValue(changes, source) {
    const { question, subItem, changeItemVisibilityCondition, changeSubItemVisibilityCondition } = this.props;
    // loadDataのときは何もしない
    ...
    if (subItem === true) {
      Object.keys(valueObj).forEach(itemId => changeSubItemVisibilityCondition(question.getId(), itemId, valueObj[itemId]));
    } else {
  ...
  cells(row, col, prop) {
    const { survey, items } = this.props;
    const cellProperties = {};
    const visibilityCondition = items.get(row).getVisibilityCondition();
  ...
  valueRenderer(instance, td, row, col, prop, value) {
    ...
    if (visibilityCondition) {
      const od = survey.findOutputDefinition(visibilityCondition.getOutputDefinitionId());
      if (!od || od.getOutputType() !== 'number' || S(visibilityCondition.getComparisonType()).isEmpty()) {
        td.classList.add('disabled');
      }
    }
...
const actionsToProps = dispatch => ({
  ...
  changeSubItemVisibilityCondition: (questionId, itemId, value) =>
    dispatch(EditorActions.changeSubItemVisibilityCondition(questionId, itemId, value)),
});
