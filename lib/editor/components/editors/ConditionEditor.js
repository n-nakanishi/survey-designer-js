  ...
  createConditionValue(childCondition) {
    ...
    switch (outputType) {
      ...
      case 'radio': {
        ...
        if (value !== '' && choices.findIndex(choice => this.getRadioValue(choice) === value) === -1) {
        ...
        return [
          ...
          >
            {unkownOption}
            <option value="" />
            {choices.map(choice => <option key={`${questionId}_option_${choice.getValue()}`} value={this.getRadioValue(choice)}>{choice.getLabel()}</option>).toArray()}
          </ExSelect>),
