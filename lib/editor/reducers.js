...
function pageReducer(survey, action, replacer) {
  switch (action.type) {
    ...
    case C.CHANGE_ITEM_VISIBILITY_CONDITION:
      return survey.updateIn(pageSelector(survey, action), page =>
        page.updateItemVisiblityCondition(survey, action.questionId, action.itemId, action.value, replacer));
    case C.CHANGE_SUB_ITEM_VISIBILITY_CONDITION:
      return survey.updateIn(pageSelector(survey, action), page =>
        page.updateSubItemVisiblityCondition(survey, action.questionId, action.itemId, action.value, replacer));
