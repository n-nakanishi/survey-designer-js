...
export default class PageDefinition extends PageDefinitionRecord {
   ...
   *
   * valueはitemIdをキーとしたObject。一度に更新するのはHandsontableのパフォーマンス問題に対応するため。
   */
  updateItemVisiblityConditions(survey, questionId, value, replacer) {
    const questionIndex = this.findQuestionIndex(questionId);
    let newState = this;
    Object.keys(value).forEach((itemId) => {
      ...
      newState = newState.updateIn(['questions', questionIndex, 'items', itemIndex, 'visibilityCondition'], visibilityCondition => (
        visibilityCondition === null ?
          VisibilityConditionDefinition.create(outputDefinitions, value[itemId], replacer)
          : visibilityCondition.updateProperties(outputDefinitions, value[itemId], replacer)
      ));
    });
    return newState;
  }
   ...
   * valueはitemIdをキーとしたObject。一度に更新するのはHandsontableのパフォーマンス問題に対応するため。
   */
  updateSubItemVisiblityConditions(survey, questionId, value, replacer) {
    const questionIndex = this.findQuestionIndex(questionId);
    let newState = this;
    Object.keys(value).forEach((itemId) => {
