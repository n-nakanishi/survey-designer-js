...
export default class PageDefinition extends PageDefinitionRecord {
  ...
  updateItemVisiblityCondition(survey, questionId, itemId, value, replacer) {
    const questionIndex = this.findQuestionIndex(questionId);
    const itemIndex = this.findItemIndex(questionId, itemId);
    ...
    const newState = this.updateIn(['questions', questionIndex, 'items', itemIndex, 'visibilityCondition'], visibilityCondition => (
      visibilityCondition === null ?
        VisibilityConditionDefinition.create(outputDefinitions, value, replacer)
        : visibilityCondition.updateProperties(outputDefinitions, value, replacer)
    ));
    return newState;
  }

  /** subItemのvisiblityConditionの更新 */
  updateSubItemVisiblityCondition(survey, questionId, itemId, value, replacer) {
    const questionIndex = this.findQuestionIndex(questionId);
    const itemIndex = this.findSubItemIndex(questionId, itemId);
    const node = survey.findNodeFromRefId(this.getId());
    const outputDefinitions = survey.findPrecedingOutputDefinition(node.getId(), true);
    const newState = this.updateIn(['questions', questionIndex, 'subItems', itemIndex, 'visibilityCondition'], visibilityCondition => (
      visibilityCondition === null ?
        VisibilityConditionDefinition.create(outputDefinitions, value, replacer)
        : visibilityCondition.updateProperties(outputDefinitions, value, replacer)
    ));
