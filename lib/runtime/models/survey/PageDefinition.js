...
export default class PageDefinition extends PageDefinitionRecord {
  ...
  updateItemVisiblityCondition(survey, questionId, itemId, attributeName, value, replacer) {
    const questionIndex = this.findQuestionIndex(questionId);
    const itemIndex = this.findItemIndex(questionId, itemId);
    if (attributeName === 'outputId' && S(value).isEmpty()) {
      // outputIdが空になった場合は、容量削減のためnullにする
      return this.setIn(['questions', questionIndex, 'items', itemIndex, 'visibilityCondition'], null);
    }

    let newState = this;
    // visibilityConditionが存在しなければ作成
    if (this.getIn(['questions', questionIndex, 'items', itemIndex, 'visibilityCondition']) === null) {
      const visibilityCondition = VisibilityConditionDefinition.create();
      newState = this.setIn(['questions', questionIndex, 'items', itemIndex, 'visibilityCondition'], visibilityCondition);
    }

    // comparisonTypeが変更になった場合はvalueも変更
    if (attributeName === 'comparisonType') {
      newState = newState.setIn(['questions', questionIndex, 'items', itemIndex, 'visibilityCondition', 'value'], null);
    }

    // outputDefinitionIdのoutputTypeが変わった場合他の値もnullに設定
    if (attributeName === 'outputDefinitionId') {
      const oldOutputDefinitionId = newState.getIn(['questions', questionIndex, 'items', itemIndex, 'visibilityCondition', 'outputDefinitionId']);
      const oldOutputDefinition = survey.findOutputDefinition(oldOutputDefinitionId);
      const newOutputDefinition = survey.findOutputDefinition(value);
      if (!newOutputDefinition || (oldOutputDefinition && oldOutputDefinition.getOutputType() !== newOutputDefinition.getOutputType())) {
        newState = newState
          .setIn(['questions', questionIndex, 'items', itemIndex, 'visibilityCondition', 'comparisonType'], null)
          .setIn(['questions', questionIndex, 'items', itemIndex, 'visibilityCondition', 'value'], null)
          .setIn(['questions', questionIndex, 'items', itemIndex, 'visibilityCondition', 'operator'], null);
      }
    }
    return newState.setIn(['questions', questionIndex, 'items', itemIndex, 'visibilityCondition', attributeName], replacer.no2Id(value));
  }
