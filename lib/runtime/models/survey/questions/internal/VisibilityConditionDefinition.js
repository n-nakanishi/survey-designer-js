...
export default class VisibilityConditionDefinition extends VisibilityConditionDefinitionRecord {
  ...
  validate(survey, node, page, question, item) {
    ...
    const outputType = outputDefinition.getOutputType();
    if (outputType === 'number' && S(this.getComparisonType()).isEmpty()) errors = errors.push(`${location}比較タイプが不正です`);
    if ((outputType === 'number' || outputType === 'radio') && S(this.getValue()).isEmpty()) errors = errors.push(`${location}比較値が不正です`);
