...
export default class VisibilityConditionDefinition extends VisibilityConditionDefinitionRecord {
  ...
  validate(survey, node, page, question, item) {
    let errors = List();
    const location = `${item.getIndex() + 1}つ目の選択肢の表示条件で`;
    ...
    if (outputType === 'number') {
      if (S(this.getComparisonType()).isEmpty()) errors = errors.push(`${location}比較タイプが不正です`);
      if (S(this.getValue()).isEmpty()) errors = errors.push(`${location}比較値が不正です`);
    }
