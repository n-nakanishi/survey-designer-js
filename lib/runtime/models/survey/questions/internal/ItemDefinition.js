...
export default class ItemDefinition extends ItemRecord {
  ...
  isVisibilityConditionMatch(survey, answers) {
    ...
    const value = visibilityCondition.getValue();

    let answerValue;
    if (visibilityCondition.getComparisonType() === 'answerValue') {
      survey.refreshReplacer(answers.toJS());
      const replacer = survey.getReplacer();
      answerValue = replacer.id2Value(visibilityCondition.getValue());
    } else {
