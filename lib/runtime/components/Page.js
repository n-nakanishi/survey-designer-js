...
class Page extends Component {
  ...
  componentDidMount() {
    ...
    this.parsley();
    $(this.pageEl).on('change keyup', 'input,select,textarea', this.setCurrentPageAnswer.bind(this));
  }
  ...
  componentDidUpdate(prevProps) {
    ...
    // parsleyのcheckboxに対するmincheckのチェックカウントがhiddenのものも含まれてしまう不具合がある
    // そのため、非表示となったcheckboxはチェックを外し、整合性をとる
    $(this.pageEl).find('input[type="checkbox"]:hidden').prop('checked', false);
  }

  setCurrentPageAnswer() {
    const answers = this.getAnswers();
    this.props.changeCurrentPageAnswers(answers);
  }
  ...
  parsley() {
    ...
    this.$parsley = $(this.pageEl).parsley({
      // デフォルト値に:hiidenを加えている。非表示項目はvalidationしない
